...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void swim(int index) {
        ...
        int pid = parentIndex(index);
        if (pid == 0) {
            return;
        }
        if (min(pid, index) == pid) {
            return;
        }
    ...
    private void sink(int index) {
        ...
        int lid = leftIndex(index);
        int rid = rightIndex(index);
        if (getNode(lid) == null && getNode(rid) == null) {
            return;
        }

        int minChild = min(lid, rid);
    ...
    @Override
    public T peek() {
        return size == 0 ? null : contents[1].item();
    }
    ...
    public void changePriority(T item, double priority) {
        ...
        int pid = parentIndex(idx);
        if (pid == 0) {
            sink(idx);
        }

        if (min(parentIndex(idx), idx) == idx) {
            swim(idx);
        } else {
            sink(idx);
        }
